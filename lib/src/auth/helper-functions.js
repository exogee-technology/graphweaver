...
const mapOperation = (operationName) => {
    const result = __1.AccessType[titleCase(operationName)];
    if (result === undefined) {
        logger_1.logger.error(`Could not map invalid operation name: ${operationName}`);
        throw new Error(__1.GENERIC_AUTH_ERROR_MESSAGE);
    }
    return result;
};
...
const buildAccessControlEntryForUser = (acl, roles) => {
    ...
    if (roles.includes(getAdministratorRoleName())) {
        return {
            [__1.AccessType.Read]: true,
            [__1.AccessType.Create]: true,
            [__1.AccessType.Update]: true,
            [__1.AccessType.Delete]: true,
        };
    }
    // Build a new AccessControlEntry representing the combined permission represented by all the roles
    // the user belongs to
    const consideredRoles = [...roles, __1.BASE_ROLE_EVERYONE];
