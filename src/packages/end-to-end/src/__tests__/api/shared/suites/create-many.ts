...
export const createManySuite = (options: DialectOptions) => {
	describe('createMany mutations', () => {
		...
		test('should create multiple artists', async () => {
			...
			const response = await request<{ createArtists: Artist[] }>(config.baseUrl)
				...
				.variables({
					input: [
						{ ...dialect.primaryKey('artistId', dialect.nextId('artist')), name: 'Artist One' },
						{ ...dialect.primaryKey('artistId', dialect.nextId('artist')), name: 'Artist Two' },
					],
				})
				.expectNoErrors();
			...
			// Order is the assertion that matters here: core pairs createMany's results with its
			// inputs positionally to wire up foreign keys, so a provider that returned them in any
			// other order would attach children to the wrong parents.
