...
export const ISOStringScalar = new GraphQLScalarType({
	...
	serialize(value: unknown): string {
		...
		if (!(value instanceof Date) && typeof value !== 'string') {
			throw new Error('ISOStringScalar can only serialize string or Date values');
		}
	...
	parseValue(value: unknown): string {
		...
		if (typeof value !== 'string') {
			throw new Error('ISOStringScalar can only parse string values');
		}

		const date = DateTime.fromISO(value);
		if (!date.isValid) {
			throw new Error(`Unable to parse ISOStringScalar '${value}'`);
		}
		return value;
	},
	parseLiteral(ast): string {
		...
		return ISOStringScalar.parseValue(ast.value);
	},
});
