...
const authLink = (router: RouterType) =>
	new ApolloLink((operation, forward) => {
		...
		return forward(operation).map((response) => {
			...
			if (redirectHeader) router.navigate(redirectHeader);
...
export const apolloClient = (router: RouterType) =>
	new ApolloClient({
		link: authLink(router).concat(httpLink),
