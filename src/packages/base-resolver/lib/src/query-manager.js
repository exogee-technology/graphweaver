...
const getFieldFromEntity = (entityName, fieldName) => {
    const metadata = base_resolver_1.EntityMetadataMap.get(entityName);
    if (!metadata)
    ...
    if (!field)
        throw new Error(`Could not locate '${fieldName}' field on entity '${entityName}'`);
    return field;
};
const entityNameFromType = (type) => type.name || type.toString();
const visit = async (currentEntityName, currentProvider, currentFilter) => {
    ...
    for (const [fieldName, value] of Object.entries(currentFilter)) {
        if (Array.isArray(value)) {
            currentFilter[fieldName] = await Promise.all(value.map(async (entry) => {
            ...
            const nextMetadata = base_resolver_1.EntityMetadataMap.get(entityNameFromType(field.getType()));
            if (nextMetadata) {
                ...
                if (result.provider.backendId !== currentProvider.backendId) {
                    ...
                    if (rest.length === 0 && (firstKey === 'id' || firstKey === 'id_in')) {
                        ...
                        currentFilter[fieldName] = { id_in: rows.map((row) => row.id) };
                    }
                }
            }
        }
...
class QueryManagerImplementation {
    constructor() {
        this.find = async ({ entityName, filter, pagination, }) => {
            const metadata = base_resolver_1.EntityMetadataMap.get(entityName);
            if (!metadata)
                throw new Error(`Could not get provider for '${entityName}' entity.`);
            logger_1.logger.trace('Handling cross-datasource queries');
            logger_1.logger.trace('Original filter: ', filter);
            const result = await visit(entityName, metadata.provider, filter);
            logger_1.logger.trace('Filter after ID flattening: ', filter);
...
exports.QueryManager = new QueryManagerImplementation();
