...
export class SchemaEntityFile extends BaseFile {
	...
	generate(): string {
		...
		if (
			isSinglePrimaryKeyEntity ||
			!this.isAutoGeneratedProperty(primaryKey) ||
			this.clientGeneratedPrimaryKeys
		) {
	...
	protected isAutoGeneratedProperty(prop: EntityProperty): boolean {
		const isSqliteIntegerPrimaryKey =
			this.databaseType === 'sqlite' && prop.primary && prop.type === 'integer';
		const isPrimaryKeyAutoGenerated =
			prop.autoincrement ||
			prop.generated ||
