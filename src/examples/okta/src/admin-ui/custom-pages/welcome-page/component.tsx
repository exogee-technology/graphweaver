...
export const WelcomePage = () => (
	<WelcomePageLayout skipPath="/Album">
		<Spacer height={30} />
		<h2 className={styles.heading}>Okta Example</h2>
		<Spacer height={30} />
		<p>This example shows how to use Okta as your authentication provider with Graphweaver.</p>
		<Spacer height={20} />
		<p>
			The principles shown here can be used to use any external authentication provider with
			Graphweaver, but we've built specific components to make it especially easy to use Okta.
		</p>
		<Spacer height={10} />
		...
		<ul className={styles.list}>
			...
			<li>A receiving page to get the token back from Okta.</li>
			...
			<li>
				...
				that use the{' '}
				<a href="https://www.npmjs.com/package/@okta/okta-auth-js" target="_blank">
					@okta/okta-auth-js
				</a>{' '}
				package directly if you prefer.
			</li>
		</ul>
		<Spacer height={10} />
		...
		<Spacer height={10} />
		<p>
			To get started, configure your app registration in Okta as per the readme, copy the
			.env.example file to just .env, add the values you get from the Okta console then run pnpm
			start. If you're getting a 500 error from your backend, the most likely cause is that you
			haven't set up the Okta values correctly.
		</p>
		<Spacer height={10} />
		<p>
			If you're stuck,{' '}
